...
!
     ...
     & ,LDIM(NDIM),NMAX,MN,ROTCNT,MINEL(2)
     ...
     &   ,DRSQI, X,Y,Z,AA,AB,AC,BA,BB,BC,CA,CB,CC,RTL(NDIM),RIL(NDIM)
     &   ,F(NDIM),R(NDIM),FT,MINDRS
...
!         Set vacuum box to maximum and minimum of specified structure
          ...
          IF( FIXLV(STNo) ) THEN
            ...
            ENDIF
c$$$          ELSEIF( SETLVR(STNo) ) THEN
c$$$            VAC(SDIM,SDIM) = LVR(STNo) 
c$$$            IF(VERB) THEN
c$$$              WRITE(6,*) " Setting vacuum dimension",SDIM
c$$$              WRITE(6,*) " to max/min of structure",STNi
c$$$              WRITE(6,*)MMN(2,SDIM,STNi),MMN(1,SDIM,STNi),VAC(SDIM,SDIM)
c$$$            ENDIF
          ELSE
            VAC(:,:) = 0
...
!                  Reduce fractional coordinates to less than 1
                   DO D=1,2 !Warning only in xy plane
                     FT = F(D)
                     IF( PBCS(D,STNo) )  F(D) = FT - ANINT( FT )
                   ENDDO
!                  Get new coordinate based on fractional positions
                   RTL(1) = AA*F(1) + AB*F(2)+AC*F(3)
                   RTL(2) = BA*F(1) + BB*F(2)+BC*F(3)
                   RTL(3) = CA*F(1) + CB*F(2)+CC*F(3)

                ! debug
                   WRITE(105,*) F(:)
                   WRITE(105,*) RTL(:)
                   WRITE(105,*) R0(:,IM,STNo) 
...
!                      Reduce fractional coordinates to less than 1
                       ...
                       ENDDO
!                      Get new coordinate based on fractional positions
                       RIL(1) = AA*F(1) + AB*F(2)+AC*F(3)
                       RIL(2) = BA*F(1) + BB*F(2)+BC*F(3)
                       RIL(3) = CA*F(1) + CB*F(2)+CC*F(3)
...
!                      Track min speeration in SDIM for minization
     ...
     &                    THEN
                           IF(DELSQ(3).LT.MINDR(SDIM) ) THEN
                            MINEL(1) = ELim
                            MINEL(2) = ELii
                            MINDR(SDIM) = DELSQ(SDIM)
                           ENDIF
                       ENDIF
...
!                Minimize speration between two structures
                 IF( MINRD(STNo).AND.MCNT(STNi).LE.MINML(STNo)  ) THEN
                  ...
                  MINDRS = SQRT( MINDR(SDIM) )
                  IF( .NOT.OVERLAP.AND.MINDRS.GT.VDBUF ) THEN 
                   Si = MINRS(STNo) 
                   CALL MAXMIN(STNo)
                   CALL MAXMIN(Si)
                   SHIFT=VDBUF - MINDRS 
                   IF(VERB) WRITE(6,*) "Minimizing speration by",SHIFT 
